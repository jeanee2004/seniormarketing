...
- **1구간(핵심 경로) 중국어 번역 추가** — `i18n` 구조를 `currentLang` 기반 범용 사전 조회로 일반화(`t()`/`applyStaticTranslations()`가 더 이상 'en'에 하드코딩되지 않음), `i18n.zh` 사전을 신설해 헤더·히어로·문제·로드맵·지도·맛집 카드/필터/정렬·상세 모달·구글 리뷰·실시간 검색까지만 번역. 2구간(설문/게임/로그인/마이페이지/리뷰작성/소개/FAQ/문의/식권)은 의도적으로 비워둬서, `t()`가 값이 없으면 자동으로 한국어 원문에 폴백하는 기존 메커니즘을 그대로 활용. 레스토랑 12곳에 `nameZh`/`descZh` 추가. 언어 선택 모달에는 원래부터 中文 버튼 UI가 있었지만 `SUPPORTED_LANGS`에 없어 "준비중"으로만 떴었는데, 이번에 `zh` 추가해서 실제로 동작하게 연결.
...
- **중국어는 1구간(핵심 경로)까지만** — 영어의 Phase 2에 해당하는 설문/게임/로그인/마이페이지/리뷰작성/소개/FAQ/문의/식권/약관 페이지는 중국어 사전이 없어 한국어로 보임(의도된 동작). 나중에 2구간도 중국어로 채우려면 `i18n.zh`에 해당 키들만 추가하면 됨 — 구조는 이미 영어와 동일하게 범용화돼 있어서 코드 수정 없이 사전만 채우면 됨.
