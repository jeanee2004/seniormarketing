...
8. **커뮤니티 SNS 그룹 개설** — 그룹 만들고 `COMMUNITY_LINK` 채우기 + QR코드 생성
9. **사장님 페이지 신설 + 접근 제어** (2026-08-26 계획 확정, 요구사항은 미정) — 가게 운영자(사장님)가
   쓸 별도 페이지/뷰를 만들어 메인 사이트와 연동할 예정. **아무나 들어가면 안 되고 허용된 사람만
   접근 가능해야 한다.** 다음 세션에서 구체화할 것: 화면에 뭘 보여줄지(저장/방문 통계, 리뷰, 식권
   예약 현황 등), 사장님 계정을 어떻게 발급/승인할지.
   - 지금 인증은 Supabase Auth 이메일 로그인(`syncAuthFromSession()`)뿐이고 **역할(role) 구분이
     없다** — 아무 이메일이나 가입하면 손주 계정이 되는 구조라 이대로는 사장님 여부를 못 가른다.
   - 구현 방향 후보: 허용된 사용자만 담는 테이블(예: `store_owners(user_id, restaurant_id)`)을 두고
     `saved_restaurants`와 같은 `auth.uid() = user_id` RLS 패턴으로 막는다 — 이미 검증된 패턴이라
     새로 설계할 필요 없이 그대로 반복하면 된다.
10. **운영 전환 시** — 커스텀 SMTP(Resend/SendGrid 등) 붙이고 이메일 인증 되살리기,
   법적 고지의 `(기재 예정)` 자리 채우기.
