...
### A. 새로 나온 아이디어 (2026-08-26 저녁)

**A1. 알레르기 경고**
사용자가 알레르기를 등록해두면, 위험할 수 있는 가게 상세를 열 때 경고를 띄운다.

- **지금은 메뉴 데이터가 사실상 없다.** 34곳 중 `detail.menu`(구성·원산지 포함)를 가진 건
  `조치원 할매국밥` 하나뿐이고 그마저 `isExample:true`인 예시다.
  → **메뉴 단위 판정은 9월 현장 조사 이후에나 가능하다.** 2단계로 나눈다.
  - **1단계(지금 가능)**: 카테고리·상호 기반의 *가능성* 경고. 예) 초밥·짬뽕·해물 → 갑각류·생선.
    `openDetail()` 앞에 관문을 하나 두고, 기존 `openConfirm({emoji,title,text,...})`을 재사용하면
    새 모달을 만들 필요가 없다(`requireLogin`/`confirmMark`와 같은 관문 패턴).
  - **2단계**: 메뉴가 채워지면 `detail.menu[].composition`을 실제로 훑어 판정.
- 알레르기 설정은 `store`(로컬)에 두고 마이페이지에 화면을 단다. 서버로 옮길 때는
  `saved_restaurants` 패턴(사용자별 행 + `auth.uid() = user_id` RLS)을 그대로 반복하면 된다.
- **문구는 반드시 "가능성"으로 쓴다.** 틀린 알레르기 정보는 안전 문제다.
  "이 집에는 갑각류가 있어요"(확정) 금지 → "포함돼 있을 수 있어요 · 주문 전 사장님께 확인하세요".

**A2. 섹션 순서 재배치 — 동의함**
현재: 히어로 → `#problem` → `#discover`(로드맵) → `#map` → `#restaurants` → `#reviews` → …
**정작 찾아온 이유인 맛집 검색·필터가 네 번째**다. 바꿀 순서:

> 히어로 → **`#restaurants`** → **`#map`** → `#problem` → `#discover` → 나머지 그대로

- 히어로는 남긴다. 처음 온 사람에게 "이게 뭐 하는 사이트인지" 한 줄이 없으면 목록만 덩그러니 보인다.
- `#map` 설명이 "핀을 누르면 상세보기로 이동"이라 **고르고 나서 지도로 확인**하는 흐름과도 맞는다.
- 섹션은 자기완결 블록이라 **`index.html`에서 잘라 옮기기만 하면 된다.** `renderCards()`와 지도
  초기화는 id로 요소를 찾으므로 순서와 무관하고, 앵커 링크도 id 기준이라 안 깨진다.

**A3. 카드 한 줄 소개를 다양하게**
신규 25곳의 `desc`는 수집 스크립트가 카테고리에서 찍어낸 **정형문 6종**이라 카페 19곳이 전부
"조치원읍 골목의 카페"다. 예: "조치원읍 골목의 느낌 좋은 카페", "호로록 후딱 마시는 카페".

- **런타임 AI는 필요 없다.** 카테고리별 문구 후보를 5~6개씩 손으로 써두고 `id` 슬러그를 해시해
  고른다. **랜덤은 금지** — 다시 그릴 때마다 문구가 바뀌면 사이트가 불안정해 보인다.
- `descEn`/`descZh`도 같은 개수로 짝을 맞춘다. `rDesc()`가 언어별 필드를 그대로 읽으므로
  **후보 배열을 3개 언어로 나란히 두고 같은 인덱스를 쓰는 구조**가 제일 단순하다.
- 톤은 바꾸되 **검증 안 된 사실 주장은 넣지 않는다**("40년 전통", "사장님이 친절한" 등 금지).
