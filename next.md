...
### A4. 지도 후속 (2026-08-26 밤)

이번에 전체화면 버그·표시 필터(전체/음식점/카페)·지도 언어는 처리했다. 남은 것:

**A4-1. 축척(스케일 바) 표시** — "이 길이가 200m입니다" 시각화.
`new Map(...)` 옵션에 `scaleControl: true` 한 줄이면 구글 기본 스케일 바가 붙는다.
기본 컨트롤이 디자인과 겉돌면 그때 커스텀을 고민하되, **먼저 기본값을 켜보고 판단할 것.**
다크모드에서 어떻게 보이는지도 같이 확인한다.

**A4-2. 현재 위치 기반 길찾기** — 사용자가 허락하면 내 위치에서 가게까지 경로를 추천.
- `navigator.geolocation.getCurrentPosition()`은 **HTTPS에서만** 동작한다(localhost는 예외).
  배포는 Vercel이라 문제없다. **권한을 먼저 묻지 말고**, "길찾기" 버튼을 누른 시점에만 요청할 것 —
  들어오자마자 위치를 묻는 사이트는 대부분 거절당한다.
- 지금 `directionsUrl(r)`은 목적지만 넘긴다. 위치를 얻으면 `&origin=위도,경도`를 붙이면 끝이라
  **코드 변경은 몇 줄**이다. 거절/실패하면 지금처럼 목적지만 넘긴다(조용히 degrade).
- 사이트 안에 경로선을 그리는 Directions API는 여전히 보류 — 사용 설정과 호출당 과금이 붙고,
  실제 안내는 어차피 지도 앱이 한다.

**A4-3. 가까운 순 정렬** — 위치를 한 번 얻으면 정렬에 "가까운 순"을 추가할 수 있다.
거리 계산은 `api/google-reviews.js`의 haversine과 같은 식이라 새로 설계할 게 없다.

### A5. 이용자 분석 대시보드 (계획 단계)

"사람들이 실제로 뭘 보고 뭘 누르는지"를 확인할 화면. **아직 구현 안 함 — 무엇을 볼지부터 정한다.**

**볼 만한 항목 (이 사이트에 실제로 의미 있는 것만)**
1. 방문자 수 / 재방문 비율 — 랜딩페이지의 기본 지표.
2. **어떤 가게 상세를 열었나 (가게별 순위)** — 이게 제일 값지다. 현장 조사 우선순위가 여기서 나온다.
3. **어떤 카테고리·필터를 썼나** — 카페가 19곳으로 쏠려 있는데 사람들이 실제로 카페를 찾는지,
   아니면 밥집을 찾는데 없어서 못 찾는지가 갈린다. 다음 수집 방향이 여기서 정해진다.
4. **검색어 중 결과가 0건이었던 것** — "있는데 못 찾은" 게 아니라 "아예 없는" 가게 목록이다.
5. 담기·방문 완료·리뷰·식권 예약 전환율 — 어디서 이탈하는지.
6. 언어별·기기별(모바일/데스크톱) 비중 — 교환학생 대상이 실제로 오는지 확인.

**어떻게 확인할까 — 두 갈래**
- **(a) Vercel Analytics** — 배포 대시보드에서 바로 본다. 스크립트 한 줄, 개인정보 수집 최소.
  **페이지뷰 수준까지만** 보인다(2~5번은 안 나온다). 지금 당장 1·6번만 알고 싶으면 이걸로 충분하다.
- **(b) Supabase 이벤트 테이블** — `saved_restaurants`와 같은 패턴으로 `events(name, props, at)` 한 장.
  2~5번을 보려면 이 길뿐이다. 쓰기는 `api/track.js`를 거치게 해서 **공개 키로 아무 이벤트나
  심지 못하게** 막는다(요약 캐시에서 secret 키를 쓴 것과 같은 이유).
  **개인 식별 정보는 넣지 않는다** — 익명 방문자 id(로컬 생성)와 이벤트 이름·가게 id 정도만.
  개인정보처리방침(`privacy.html`)에 수집 항목을 적어야 한다. 지금 문서에는 이 내용이 없다.
- 화면은 사장님 페이지(C5)와 같은 접근 제어 문제를 공유한다 — **역할 구분이 아직 없다.**
  둘을 같이 설계하는 게 낫다.

**추천 순서**: (a)를 먼저 켜서 사람이 오는지부터 보고, 트래픽이 생기면 (b)로 2~4번을 붙인다.
아무도 안 오는 상태에서 이벤트 테이블부터 만들면 빈 대시보드만 남는다.
### A6. 언어 추가 시 손봐야 하는 곳 (하드코딩 지점)

**지원 언어는 한/영/중 3개에서 멈추지 않는다.** 새 문자열은 3개 배열을 데이터에 나란히 박지 말고
i18n 사전 키 하나로 두고 `t(key)`로 꺼낼 것(카드 소개 문구 `descVar…`가 그 방식이다).

언어를 하나 늘릴 때 실제로 고쳐야 하는 곳은 다음이 전부다:

| 위치 | 무엇 |
|---|---|
| `script.js` `SUPPORTED_LANGS` | 사이트가 인정하는 언어 목록 |
| `script.js` `i18n.<lang>` | 사전 블록 추가 |
| `script.js` `mapsLanguage()` | 구글 지도 언어 코드 매핑(zh는 `zh-CN`) |
| `privacy.html` / `terms.html` | 인라인 `DICTS`와 `CYCLE`(각 페이지가 독립 스크립트를 쓴다) |
| `api/review-analysis.js`, `api/visit-verify.js` | `['ko','en','zh']` 화이트리스트 |
| 가게 데이터 `nameEn`/`nameZh` | **여기만 데이터에 박혀 있다** — 34곳 × 언어 수라 부담이 크다 |

마지막 줄이 유일한 약점이다. 언어를 더 늘리기 전에 가게 이름·설명도 사전 키 방식으로 옮기는 게
낫다(`rName()`/`rDesc()`가 이미 한 곳에서 갈라주므로 그 두 함수만 바꾸면 된다).
### B. 코드 작업
...
B2. ~~**글로벌 내비게이션 + 모바일 햄버거**~~ — **완료.** 헤더에 섹션 링크 4개
   (맛집·지도·소개·함께하기)를 넣고, 1024px 미만에서는 햄버거로 접는다.
   - 패널은 모달이 아니라 헤더에 붙는 드롭다운이라 `.survey-overlay` 구조를 쓰지 않는다.
     링크를 누르면 바로 닫힌다 — 같은 페이지 앵커라 패널이 남으면 도착지를 가린다.
     바깥 클릭·Esc로도 닫힌다.
   - **CSS 순서 함정**: `.hamburger{display:flex}`를 `@media(min-width:1024px)` 뒤에 두면
     같은 특이도라 나중 규칙이 이겨서 넓은 화면에도 햄버거가 남는다. 기본값을 먼저 두고
     미디어쿼리로 끄는 순서여야 한다(실제로 한 번 겪었다).
B3. **히어로 영상 오버레이 미세조정** — 본문 문단이 오버레이가 가장 옅은 자리에 걸린다는 지적이
   있었다. 지금도 읽히지만 거슬리면 `style.css` 끝의 `--veil-*` 값만 손보면 된다.

### C. 코드 작업 아님 (등록·수집·개설)

C1. **Vercel 환경변수** — 배포본에서 아직 안 되는 것들의 원인이다.
   - ~~지도 키~~ — **해결.** `/api/map-key`가 서버 환경변수(`Google_Javascript_API_key`)를 읽어
     브라우저에 내려주고, `resolveMapsKey()`가 `config.js`(로컬) → 이 엔드포인트(배포) 순으로 찾는다.
     `config.js`를 막은 상태에서 지도 생성·마커 34개 확인 완료.
     **키가 숨겨지는 건 아니다** — Maps JS 키는 브라우저 요청 URL에 그대로 실린다.
...
### D2. 히어로 배경 영상이 더 잘 보이게 오버레이 조절 (B3의 구체화) — ✅ 2026-08-29 세션에서 완료

아래는 착수 당시 인계 내용(그대로 유지). 실제로 적용한 최종 값과 측정치는 파일 하단
"2026-08-29 세션" 항목 참고 — 결론만 먼저 적으면:
- 라이트 데스크톱 `--veil-1:88%; --veil-2:87%; --veil-3:54%`
- 다크 데스크톱 `--veil-1:83%; --veil-2:82%; --veil-3:68%`
- 모바일(≤900px) 균일값: 라이트 `88%`, 다크 `81%`
- 최악 프레임 기준(루프 이음매 저노출 프레임 포함) h1·본문 대비비 전부 WCAG 기준 대비
  5% 이상 여유 확보(자세한 수치는 하단 참고). **eyebrow 뱃지만 예외** — `.eyebrow`의 9% 불투명
  배경 자체가 영상과 무관하게 순수 `--base-bg` 위에서도 약 3.6:1로 사이트 전역에서 이미
  AA(4.5:1) 미달이라(pre-existing), 이번 작업 범위 밖으로 남겨뒀다(후속 필요).

`style.css`만 건드리는 작업이다. 현재 값(`.hero-media`, 1479줄 근처):

- 라이트: `--veil-1:94%; --veil-2:89%; --veil-3:54%;` — 카피가 놓이는 왼쪽이 94%라
  영상이 사실상 안 보인다. 이걸 얇게 만드는 게 목표.
- 다크: `--veil-1:94%; --veil-2:88%; --veil-3:68%;` + `video{filter:brightness(0.68) saturate(0.9)}`
- 모바일(≤900px): 1단 가운데정렬이라 균일하게 `91%`.
- **`color-mix` 미지원 브라우저용 폴백 `rgba(247,244,238,0.94/0.89/0.54)` 줄이 따로 있다.**
  veil 값을 바꾸면 이 폴백도 같이 바꿔야 한다 — 놓치기 쉽다. 모바일 쪽에도 폴백이 따로 있다.

**눈대중으로 숫자를 낮추지 말 것.** 히어로에는 긴 본문 `<p>`가 얹혀 있어 가독성이 먼저다. 방법:

1. `<video>`를 `<canvas>`에 `drawImage`해서 여러 시점의 실제 프레임 픽셀을 샘플링(밝은/어두운 프레임 모두).
2. 각 텍스트의 x좌표에 해당하는 veil 농도로 합성 배경색을 구한다(그라디언트라 위치마다 다르다).
3. WCAG 대비비 계산 — 본문·eyebrow는 **4.5:1 이상**, h1 같은 큰 글씨는 3:1 이상.
   **모든 샘플 프레임에서 최악값이 기준을 넘어야 한다.**
4. 그 제약 안에서 최대한 얇게.

루프 이음매에서 `.hero.hero-loop-edge .hero-media video{opacity:0.32}`로 0.38초간 옅어진다 —
**영상이 가장 안 보이는 최악 프레임**이니 대비 계산에 포함할 것.

검증 시 헤드리스에서 영상이 자동재생되지 않으면 `--autoplay-policy=no-user-gesture-required`를 쓰고,
`.hero-video-on` 클래스가 실제로 붙었는지 확인할 것 — 안 붙으면 측정이 무의미하다.
라이트·다크·모바일(390px) 각각 변경 전/후 스크린샷을 남긴다.

### 이 세션에서 이미 끝나서 푸시된 것 (다시 하지 말 것)

- `6d5342b` 주소·전화 복사 버튼을 데이터 따라 자동으로 붙게 통일 (`infoRowHtml()`)
...
### 별도 보류 — D1(프랑스어)은 그대로 남아있음, D2(히어로 영상)는 완료

이번 사장님 페이지 작업과 무관하게, D1은 아직 처리 전이다(섹션 내용 그대로 다음 세션에서 이어서
진행할 것). D2(히어로 영상 오버레이)는 이후 세션에서 완료했다 — 아래 "2026-08-29 세션" 참고.
- `3c0e57f` 검색 오타 보정 — "이런 걸 찾으셨나요?" 제안

## 2026-08-29 세션: D2(히어로 영상 오버레이 얇게) + 버그 리포트 2건(모바일 무재생·데스크톱 확대) — `style.css` / `index.html`

### D2 — veil 얇게, 실측 대비비로 결정

방법은 위 인계 내용 그대로: 헤드리스 크롬(`--autoplay-policy=no-user-gesture-required`)으로
`.hero-video-on` 확인 후, `<video>`를 `<canvas>`에 그려 12개 시점(밝은/어두운 장면 + 루프 이음매
저노출 0.32 프레임 포함) × eyebrow/h1/h1 span/본문 각 요소의 실제 x·y 위치 그리드(요소당
9~15점)를 픽셀 샘플링. 100deg 그라디언트의 실제 위치별 veil 농도를 좌표 기하로 계산해 합성
배경색을 구하고, `--ink-soft`/`--slate-dark`/`--red`(라이트·다크 각각) 대비 WCAG 대비비를 계산해서
최악값이 기준(본문·eyebrow 4.5:1, h1·h1span 3:1)을 5% 이상 여유로 넘는 가장 얇은 값을 탐색했다.
데스크톱은 920px·1440px(2단 그리드 폭 양끝) 둘 다, 모바일은 375·390·430px 셋 다 통과하는 값만
채택. `.hero`의 라이트 tint(radial-gradient, 최대 10% 불투명)는 무시하고 "영상 없음 = 순수
`--base-bg`"로 근사했다(보수적으로 안전한 쪽 근사 — 실제로는 그것보다 항상 더 유리하다).

**최종 값**(기존 → 신규):
- 라이트 데스크톱: `--veil-1:94%→88%; --veil-2:89%→87%; --veil-3:54%(유지)`
  최악 대비 — h1span 3.17:1(920px)/3.21:1(1440px), 본문 4.78:1/4.90:1 (기준 3:1 / 4.5:1)
- 다크 데스크톱: `--veil-1:94%→83%; --veil-2:88%→82%; --veil-3:68%(유지)`
  최악 대비 — h1span 4.06:1/4.20:1, 본문 4.98:1/4.73:1
- 모바일 균일값: 라이트 `91%→88%`, 다크 `93%→81%`
  최악 대비 — h1span 3.20:1(라이트)/3.91~4.32:1(다크), 본문 4.86:1(라이트)/4.78~4.83:1(다크)
- `color-mix` 미지원 폴백 rgba도 전부 같이 갱신(라이트 데스크톱 첫 줄, 모바일 라이트 rgba 줄).

**eyebrow는 기준 미달로 남았다(이번 범위 밖)** — `.eyebrow`의 배경이 `rgba(214,67,46,0.09)`
(9% 불투명 빨강 틴트) 뿐이라, 영상이 전혀 없는 순수 `--base-bg` 위에 올려도 텍스트 자체와의
대비가 약 3.6:1로 AA 기준(4.5:1) 미달이다 — 즉 히어로 영상과 무관하게 `.eyebrow` 컴포넌트
자체가 사이트 전역(다른 섹션의 eyebrow 뱃지 포함)에서 이미 이 문제를 안고 있었다. veil을
얇게 하면서 약간 더 나빠지긴 했지만(라이트 데스크톱 기준 3.12→2.86 정도) 근본 원인이 아니라서
이번 작업에서 고치지 않았다. **후속 필요**: `.eyebrow`의 배경 불투명도를 올리거나 텍스트색을
바꿔야 하는데, 사이트 전역에 쓰이는 공유 컴포넌트라 히어로만 손대려면 `.hero .eyebrow` 별도
오버라이드가 필요함 — 다음 세션 후보로 남김.

### 버그 리포트 2건 — 같은 파일이라 이어서 처리

**1) 모바일 실기기에서 영상이 아예 안 뜬다는 리포트.** `.hero{position:relative}`,
`.hero-media{position:absolute;inset:0}`, `.hero-video-on` 토글 로직을 다 확인했지만 헤드리스
크롬(375/390/430px)에서는 전부 정상 재생·정상 렌더(`hero-media` opacity:1, 크기도 0 아님) —
CSS 포지셔닝 버그는 재현되지 않았다. Chrome은 muted 영상은 기본적으로 autoplay를 허용해서
"autoplay 차단"도 이 환경에서는 재현 불가(실기기의 iOS 저전력 모드·카톡 인앱브라우저 등 특정
조건이 원인일 가능성이 높은데 이 환경에서 흉내낼 수 없었다). 유력한 진짜 원인으로 지목한 건
`preload="metadata"` + play() 실패 조합 — metadata만 요청한 상태에서 자동재생이 막히면 실제
프레임 데이터를 더 안 당겨와서 `loadeddata`가 영영 안 붙을 수 있다. **조치**: `preload="auto"`로
변경(`index.html`) — play() 성패와 무관하게 첫 프레임을 미리 받아두게 해서, 재생이 막혀도 최소한
정지 프레임은 배경으로 뜨게 하는 폴백. 완전한 원인 규명은 못했다는 점을 남겨둔다(실기기 재현
불가 환경의 한계) — 배포 후에도 재현되면 다음 후보는 카톡 인앱 브라우저 UA 감지 후 poster
이미지로 대체하는 정도.

**2) 노트북 창 폭을 줄이면 영상이 확대돼 보인다는 리포트.** 실측으로 확인됨 — `.hero-media`가
`.hero`의 콘텐츠 높이(카피 블록 높이)에 그대로 끌려가는데, 2단 그리드가 유지되는 900~1440px
구간에서 폭은 줄어도 높이는 거의 안 줄어(718~750px 사이) 박스 비율이 영상 원본 비율(16:9=1.78)과
크게 벌어진다 — 1440px에서는 영상 가로폭 100%가 보이지만 920px에서는 72%만 보임(28% 크롭,
`object-fit:cover`가 정상 동작한 결과지만 시각적으로는 "확대"로 읽힘). `object-fit:cover` 자체는
그대로 두고(교체 대상 아님), `.hero-media`가 커버할 수 있는 최대 높이를 영상 원본 비율로
못박았다: `@media(min-width:901px){ .hero-media{max-height:calc(100vw / var(--hero-ratio, 1.778))} }`.
`--hero-ratio`는 `index.html` 인라인 스크립트가 `loadedmetadata`에서 `videoWidth/videoHeight`로
채워서, 다른 비율 영상으로 교체해도 CSS 숫자를 다시 맞출 필요가 없다. 카피 텍스트(h1·본문)는
`.hero-grid` 쪽이라 이 max-height의 영향을 받지 않고(별도 요소), 실측으로 텍스트 하단(본문
bottom)이 새 캡 경계보다 항상 위에 있는 것도 901~1440px 전 구간에서 확인했다(최소 여유 37px
@901px) — 즉 대비비 재계산 없이 위 D2 수치가 그대로 유효하다. 결과: 901~1440px 전 구간에서
크롭 없이(비율이 정확히 일치하는 폭에서는 0% 크롭, 그 외엔 세로 방향만 약간 크롭) 렌더됨을
헤드리스 스크린샷(901/920/960/1024/1100/1200/1333/1440px)으로 확인.

검증: 헤드리스 크롬으로 라이트/다크 데스크톱(1440px)·모바일(390px) 스크린샷, `.hero-video-on`
토글 확인, 콘솔 에러 0(지도 API 키 미설정 경고만 있음, 기존에도 있던 것). 스크린샷 파일은
작업용이라 커밋하지 않음.

## 2026-08-29 세션(이어서): 라이트 모드 히어로 veil 재검증 — 더 얇게 갈 여지 없음, 값 유지

같은 세션 중 사용자 리포트: "다크모드일때는 히어로 영상이 좀 더 보이는 것 같은데 라이트
모드일때는 잘 안보이고 대비도 잘 안돼." 두 갈래로 나눠 처리했다.

**1) `.eyebrow` 대비 미달 — 확정 원인, 커밋 `dbaf25b`로 완료.** 라이트 모드에서만 `.eyebrow`
배지의 텍스트/배경 대비가 3.61:1로 AA(4.5:1) 미달이었다(다크는 5.22:1로 문제없음) — 배경
필(`rgba(214,67,46,0.09)`)은 라이트/다크 공통 고정값인데, 라이트가 다크보다 밝은 `--red`를
텍스트색으로 그대로 썼기 때문. `color`를 `--red-dark`로 바꿔 라이트 4.79:1/다크 6.58:1로
해결(위 D2 세션에서 "eyebrow는 범위 밖"이라고 남겨뒀던 후속 항목이 이걸로 완료됨).

**2) 히어로 영상 자체가 라이트에서 잘 안 보인다는 절반 — 재측정했지만 값은 그대로 뒀다.**
라이트 veil(`--veil-1:88%; --veil-2:87%; --veil-3:54%` 데스크톱, 모바일 균일 `88%`)을 D2보다
훨씬 촘촘하게 다시 쟀다:
- 10.005초 루프 전체를 0.25초 간격(41개 시점)으로 헤드리스 크롬 + CDP로 순회(D2는 12개 시점).
- 텍스트 영역을 `getBoundingClientRect()` 하나가 아니라 `Range.getClientRects()`로 "실제 렌더된
  줄" 단위 사각형으로 뽑았다 — 여러 줄로 꺾이는 h1·본문을 통짜 bounding box로 재면 줄과 줄
  사이 여백(글자가 없는 자리)까지 텍스트 영역으로 착각해 엉뚱하게 낙관적이거나 비관적인 값이
  나올 수 있다.
- 각 줄마다 그리드 점을 찍고, **점들을 평균하지 않고 개별 픽셀의 최악값**으로 대비비를
  계산했다. 평균은 국소적으로 아주 어두운 반점(예: 영상 속 그림자·검은 그릇)을 주변 밝은
  픽셀과 섞어 실제보다 낙관적인 숫자를 만든다 — 실제로 처음에 그리드 평균으로 쟀을 때는
  84%/83% 같은 더 얇은 값도 통과하는 것처럼 나왔는데, 픽셀 단위로 다시 재니 그 여유가 사라졌다.

**결론(라이트, 데스크톱 920/1440px 공통 최악값)**: h1 span 3.14:1/3.17:1(기준 3:1), 본문
4.77:1(기준 4.5:1) — 이미 안전 여유(권장 5%) 경계에 딱 걸쳐 있다(약 4.8~5.7%). 최악 프레임은
영상 속 검은 뚝배기·냄비가 정확히 본문·h1 자리에 걸치는 순간(예: 1440px에서 t≈4.3초)이었다 —
veil을 잠깐 꺼서 raw 프레임을 스크린샷으로 직접 확인해 샘플링 버그가 아니라 실제 영상 내용임을
눈으로 검증했다. 이 상태에서 88%/87%를 조금이라도 더 낮추면 이 프레임에서 AA 기준(3:1/4.5:1)
자체가 깨질 위험이 있다. 모바일(375/390/430px, 균일값)도 같은 방식으로 재검증: 88%가 최악
3.16~3.19:1로 마찬가지로 87%까지만 낮춰도 기준 미달 프레임이 나왔다.

**그래서 라이트 데스크톱/모바일 veil 값은 손대지 않았다** — "더 얇게 만들 수 있는 실측 근거가
없다"가 이번 재검증의 정직한 결론이다. 억지로 낮추면 실제로 안 보이는 순간이 생긴다. 다크 모드는
이번 리포트 대상이 아니어서(사용자가 다크는 괜찮다고 함) 그대로 뒀다. `--veil-3`(오른쪽 캐릭터
카드 쪽, 54%)도 이번 불만과 무관해서 손대지 않았다.

**작업 중 도구 관련 메모(다음 세션 참고)**: 이번 세션에서 두 번, `claude-in-chrome` 확장(이미
떠 있는 인터랙티브 크롬 탭을 확장으로 조종하는 방식)으로 히어로 `<video>`를 다루려 하니
`readyState`가 0에서 10초 넘게 멈춰(`NETWORK_LOADING`에서 응답 없음) 진행이 안 됐다 — 로컬
`node dev-server.js`와 배포된 프로덕션 사이트 둘 다에서 재현됐고, 그 상태에서 `video.play()`를
호출하면 CDP `Runtime.evaluate` 자체가 45초 타임아웃까지 멈췄다. 반면 이 세션에서 실제로 값을
측정한 방법 — CLAUDE.md에 적힌 대로 **전용 헤드리스 크롬 프로세스를 직접 띄우고
(`--headless=new --remote-debugging-port=...`) 그 프로세스의 디버그 포트에 raw WebSocket으로
직접 붙어 CDP를 호출하는 방식** — 은 두 번 다 문제없이 즉시 재생되고(`readyState:4`,
`duration:10.005`) 프레임 캔버스 샘플링까지 정상 동작했다. 원인은 특정하지 못했지만(확장이
...
## 2026-09-01 세션: owner.html 폼 보완 · 지도에서 위치 보기 · 가게 이름/설명 i18n 이전

이전 세션의 "남은 것" 3개 중 값/외부 결정이 필요 없는 3번(owner.html 폼 디테일)부터
처리하고, 이어서 사용자 제안으로 지도 관련 UX 하나, next.md A6이 지적하던 다국어
마지막 약점 하나를 마저 처리했다. 셋 다 커밋·푸시 완료, 프로덕션 배포까지 확인.

- **owner.html 로그인/가입 폼 보완** (커밋 `c4a254d`) — 이메일 형식·비밀번호 확인 불일치 시
  `.is-invalid` 강조 + focus, 재입력 시 실시간 클리어, 가입 시 이름·비밀번호 확인 필수 검증을
  index.html 메인 인증 모달 수준으로 맞췄다. `authErrorText()`에 이메일 미인증/rate-limit/형식
  오류 케이스를 추가해 `authErrorMessage()`와 커버리지를 동일하게 하고, 가입 후 세션이 없는
  경우(메일 인증 대기)도 안내하게 했다. owner.html은 여전히 언어 토글이 없어 한국어 고정이고,
  다국어를 붙이는 건 별개 작업으로 남아있다(아래 목록에서 제외 — 지금 당장 필요한 항목이 아님).
- **길찾기·텍스트 길안내 → "지도에서 위치 보기"로 교체** (커밋 `9e11542`) — 상세 모달의 외부
  지도 이동/도보 경로 텍스트 버튼을 없애고, 클릭 시 지도 섹션으로 스크롤 + 해당 가게 마커로
  지도 중심 이동(zoom 18) + bounce 애니메이션 버튼 하나로 통합했다. 마커가 카테고리/가격/지도
  필터에 가려질 수 있어 이동 전 필터를 전체로 초기화한다. 마커를 id로 바로 찾도록
  `gMarkerById`를 추가했다. `gmap.panTo()`가 실제로는 지도를 안 움직이는 문제가 있어
  `gmap.setCenter()`로 교체(원인 미상 — panTo 자체가 이 프로젝트 지도 설정에서 동작하지 않았다,
  setCenter는 즉시 반영되어 정상 동작 확인). 더 안 쓰는 `api/walk-route.js`와 관련
  i18n 키(`detailDirections`/`detailRouteBtn` 등)·CSS(`.detail-route*`/`.route-text-*`) 정리.
  **함께 보류한 것**: "약도 보기"(카드별 50m 미니지도)는 Google Static Maps API가 비활성화
  상태라 이번엔 보류. 재검토한다면 API 키가 필요 없는 OpenStreetMap 정적 타일 이미지 방식이
  유력(next.md 8월 25일 세션 참고 — 이 사이트가 예전에 Leaflet/OSM을 실제로 썼던 적이 있다).
- **가게 이름·설명 다국어를 데이터 필드에서 i18n 사전으로 이전** (커밋 `3a0a171`, next.md A6의
  마지막 약점 해소) — `restaurants[]`의 `nameEn`/`nameZh`/`nameEs`/`nameFr`,
  `descEn`/`descZh`/`descEs`/`descFr`를 `i18n.<lang>` 사전의 `name_<id>`/`desc_<id>` 키로
  옮겼다. `rName()`/`rDesc()`는 `rCat()`과 같은 `t(key)` 조회 한 줄로 단순화. 정형문 27곳
  (`descVariant()`가 이미 en/zh/es/fr 전부 처리하던 곳)의 `descEn`/`descZh`는 사전으로 옮기지
  않고 그냥 제거했다 — `descVariant()`가 `rDesc()`보다 먼저 걸려서 원래도 참조되지 않던 죽은
  데이터였다. 실제 desc가 있는 10곳만 사전으로 이전. `rName()`/`rDesc()`를 거치지 않고 모든
  언어를 한 번에 훑던 3곳(카카오 오타 보정 제안, 헤더 통합검색 가게/식권 키워드)은 새
  `nameAllLangs()` 헬퍼로 통일. 1회성 Node 마이그레이션 스크립트로 기계적 치환(손으로 37곳
  옮기면 오타 위험) 후 필드 제거 수·사전 키 수 대조, 5개 언어 렌더링 일치, 검색·퍼지매칭
  회귀까지 확인.

**검증**: 세 작업 모두 `node --check` + `dev-server.js` + 헤드리스 Chrome(CDP)으로 로컬 확인,
콘솔 예외 0건. 지도 기능은 배포 후 프로덕션(seniormarketing.vercel.app)에서도 재검증 완료.

## 2026-09-02 세션: 독일어·일본어 추가, 닐슨 휴리스틱 개선 3건, 룰렛 원형 휠 전면 개편, 카카오 검색↔카드 연결, 관리자 설정 완료

이번 세션은 여러 요청이 이어져서 커밋이 많다. 전부 로컬 `node --check` + `dev-server.js` +
헤드리스 Chrome(CDP)으로 검증 후 배포까지 확인했고, 콘솔 예외는 매번 0건이었다.

- **독일어(de)·일본어(ja) 언어 추가** (`5682d09`, `2ccd334`) — en/zh/es/fr와 완전히 동일한
  방식으로 사이트 전체(메인 페이지 전 모달 + privacy.html/terms.html) 커버. `SUPPORTED_LANGS`에
  추가, `mapsLanguage()`/`pBenefit()`/`wonSuffix()`/`passUnit()`/`nameAllLangs()`/설문 문항에
  언어 분기 추가. api/review-analysis.js·api/visit-verify.js 화이트리스트에도 추가. 이제 지원
  언어는 한/영/중/스페인/프랑스/독일/일본 7개.
- **닐슨 휴리스틱 기반 개선 3건** (사용자가 4항목 제시, ②④는 이미 구현돼 있어서 제외) —
  ① 필터 바에 상시 "필터 초기화" 버튼 추가(`16f98d1`, 기존 `resetFilters()`/`filterReset` 키
  재사용), ③ 실제 가게 카드에 🗺️ 지도 포커싱 버튼 추가(`19b7952`, 기존 `focusMapMarker()` 재사용
  — 예전엔 상세 모달을 열어야만 가능했음).
- **반응형 감사 서브에이전트 실행 → 헤더 겹침 버그 발견·수정** (`72c9c99`) — 320~372px·
  403~456px 폭에서 로그인(👤) 버튼이 화면 밖으로 밀려 탭 불가능했다. `.nav-wrap`에 456px 이하
  media query로 2줄 wrap 추가, 320~1024px 전 구간 오버플로 0 확인. 그 외 카드그리드·필터바·
  모달·지도·푸터는 전부 정상이었음(감사 결과 상세는 세션 로그 참고).
- **메뉴 룰렛을 슬롯머신식 텍스트에서 실제 원형 휠로 전면 개편** (`72c9c99`~`d87d78a`, 커밋
  4개에 걸쳐 반복 수정) — conic-gradient + transform:rotate 기반, 외부 라이브러리 없음.
  **칸 수(2~12, 별도 숫자칸)와 메뉴 목록(칩 입력)을 완전히 분리** — 메뉴가 칸보다 적으면
  순환 반복. 당첨 칸을 먼저 정하고 회전각을 역산해 정중앙 착지, 다시 돌리기마다 누적각을
  앞으로만 더해 스냅백 없음.
  - 사용자가 스크린샷으로 버그 3건을 잡아냈고 전부 실제 버그였다(자동화 좌표 검증이 놓친
    것들이라 **화면을 직접 봐야 잡히는 종류**였다는 게 교훈):
    1. `.roulette-wheel-label`이 `height:0`+`overflow:hidden`이라 라벨 텍스트가 완전히
       잘려 안 보였음(회전 후 bounding box는 정상 측정되어 좌표 검증으로는 못 잡음) → 제거.
    2. "거꾸로 보이는 글자 보정" 로직이 바깥 스포크의 `justify-content`를 바꿔서 왼쪽 절반
       칸의 라벨이 테두리가 아니라 허브(중심) 쪽에 몰려 겹쳐 보였음 → 바깥 스포크는 항상
       테두리 쪽(flex-end) 고정, 안쪽 텍스트 span만 제자리에서 180도 추가 회전으로 교체.
    3. `--red`/`--red-dark` 색이 너무 비슷해 옆칸이 하나로 뭉쳐 보임(3칸에서 특히 확연)
       → `--red`/`--slate-surface`/`--slate` 3색으로 교체, 라벨은 흰 글씨+그림자.
    4. 한글 IME 조합 중 Enter로 "국밥" 추가 시 조합 중간값("밥")과 최종값이 둘 다 칩으로
       들어갔음(`e.isComposing` 미확인) → 체크 추가.
  - 당첨 결과 텍스트도 1.2rem→2.2rem으로 키우고 팝인 애니메이션 추가(`d87d78a`).
- **카카오 실시간 검색 결과 ↔ 실제 가게 카드 연결** (`6619cab`) — 검색한 가게가 이미
  `restaurants[]`에 있으면 결과 카드에 "이미 소개된 가게예요 · 카드에서 보기" 버튼이 붙어
  `openDetail()`로 바로 연결된다. `findCuratedMatch()`는 카카오가 이미 정확한 상호명을 주므로
  fuzzyMatch(철자 보정용, 오탐 위험) 대신 공백 제거 후 단순 포함 관계만 본다. **주의: 이건
  CLAUDE.md가 금지한 "헤더 검색↔실시간 검색 자동 연동"과는 다른 것이다** — 그건 여전히 안 함,
  이건 "실시간 검색 결과↔카드 그리드"를 잇는 별개의 기능.
- **관리자 마이페이지 인프라 정상화** — `SUPABASE_SECRET_KEY`를 `.env.local`(로컬)과 Vercel
  환경변수(배포본, 사용자가 직접 등록+재배포)에 채웠다. `SUPABASE_URL`/`ADMIN_EMAIL`도 Vercel에
  등록 확인. `/api/admin-contacts` 응답이 `500 server_missing_config` → `403 forbidden`으로
  바뀐 것까지 curl로 확인(403은 미인증 요청에 대한 정상 응답이라 설정 자체는 이제 완전함을 뜻함).
  **다음 세션에서 이어받을 것**: 실제 `jeaneeyi2004@gmail.com` 계정으로 로그인해 마이페이지에
  "관리자" 탭이 뜨는지, 문의 목록이 보이는지, "사장님으로 등록" 승인 플로우가 실제로 도는지는
  아직 검증 못 함(로그인 비밀번호가 필요해 이 세션에서 자동화 불가) — 아래 "남은 것" 1번 참고.

## 남은 것 — 다음 세션

1. **관리자 승인 플로우 실제 로그인 검증.** 서버 설정(`SUPABASE_SECRET_KEY`/`SUPABASE_URL`/
   `ADMIN_EMAIL`)은 위 세션에서 Vercel까지 완료·확인했다. 남은 건 실제 사용자 동작 확인뿐:
   (a) `jeaneeyi2004@gmail.com`으로 로그인 → 마이페이지에 "관리자" 탭이 뜨는지,
   (b) 목록에 실제 문의가 보이는지, (c) "사장님으로 등록" 클릭(미가입 이메일 →
   `not_signed_up` 안내, 가입된 이메일 → 실제 `store_owners` 행 생성을 MCP로 대조),
   (d) 연락함/거절 상태 변경. 이게 끝나야 owner.html 접근 제어(C5)와 관리자 승인 화면이
   실제로 맞물려 돌아가는 걸 증명한 게 된다.
...
- **owner.html에 다국어(i18n)가 아직 없다** — 한국어 고정 최소 구현. index.html처럼 언어 토글을
  붙이려면 `script.js`의 `i18n`/`t()`와 별개로 owner.html 전용 사전을 새로 만들어야 한다(현재
  구조상 `script.js`를 로드하지 않으므로).
- **"약도 보기"(카드별 50m 미니지도) 보류 중** — Google Static Maps API 비활성화가 걸림돌.
  API 키 없이 되는 OpenStreetMap 정적 타일 방식으로 재검토 가능.
- **owner.html에도 이제 한/영/중/스페인/프랑스/독일/일본어 7개 언어가 적용돼 있지 않다** —
  index.html은 이번 세션까지 7개 언어 전부 갖췄는데 owner.html만 그대로 한국어 고정이라
  격차가 더 벌어졌다. 위 owner.html i18n 항목과 같이 처리하는 게 효율적.
