...
#### Reviews and 손주 식권 예약 have since moved to Supabase too

Same pattern as marks, in `public.reviews` (`supabase/reviews.sql`) and `public.pass_orders` (`supabase/pass_orders.sql`) — `pushReview()`/`pushReviewDelete()`/`deleteAllMyReviews()`/`loadCommunityReviews()` and `pushPassOrder()`/`deletePassOrder()`/`deleteAllPassOrders()`/`pullPassOrders()` are the only functions that talk to those tables, all fire-and-forget. One difference from marks: **reviews are a public feed, not a per-user list** — `select` RLS is open to everyone (like Google reviews), only `insert`/`delete` are restricted to `auth.uid() = user_id`. `pass_orders` follows the saved-restaurants shape exactly (select/insert/delete all owner-only).

#### 사장님(store owner) role — table exists, no signup/approval flow yet (next.md C5)

`public.store_owners` (`supabase/store_owners.sql`) marks "this Supabase Auth user owns this restaurant" — `user_id` + `restaurant_id` (the same slug marks/reviews use). **RLS deliberately has no insert/update/delete policy at all**: a logged-in user can `select` only their own rows, but nobody — including an authenticated owner — can write to this table from the client. Rows are added only by the admin, directly in the Supabase dashboard (Table Editor or SQL Editor bypass RLS). This is intentional, not a gap to fill with a self-serve request form: real store-owner accounts don't exist before the September on-site survey (see C3), so there's nothing yet for a self-serve flow to protect.

`owner.html` reads this table client-side (its own inline Supabase client + a small login/signup form, since it doesn't load `script.js` — same reasoning as `privacy.html`/`terms.html`): logged out → login/signup form; logged in with no `store_owners` row → "문의를 남기면 연결해드립니다" pointing at the existing 제휴 문의 CTA; logged in with a row → "인증된 사장님" state naming the linked restaurant(s). The actual registration/dashboard screens below that panel are still the mockup from `9e46100` (`is-soon`, disabled) — this only gates *who counts as a verified owner*, it doesn't make the dashboard functional yet.

The 사장님 제휴·입점 문의 submission (`contactTypes.partnerStore`, routed through `submitContact()`) now also persists to `public.contact_submissions` — see the Known gaps note above. The admin reviews new applications there, then manually inserts the matching `store_owners` row once verified.
...
- Login, **saved/visited marks**, **reviews**, and **손주 식권 예약** are all real and server-backed (Supabase Auth + `saved_restaurants`/`reviews`/`pass_orders`, see Persistence above) — all four follow a user across devices now. The PRD originally scoped these behind "로드맵으로 분리," but they've since been built; treat this doc, not that section of the PRD, as current.
- Language switching (ko/en/zh/es/fr, real i18n dictionaries) and AI review summarization (`api/review-analysis.js`, Gemini) are also both real, not stubs, despite the PRD listing them as roadmap-only — same caveat as above.
- The 손주 힘 보태기 / 지역 확장 / 사장님 제휴 문의 forms (`extra.md` §7) show a confirmation screen and **also** persist to `public.contact_submissions` (`submitContact()`, insert-only RLS — nobody, including the submitter, can read it back; the admin checks the Supabase dashboard Table Editor, which bypasses RLS). No outbound notification (email/SMS) exists yet — that's still a manual "go check the dashboard" step.
